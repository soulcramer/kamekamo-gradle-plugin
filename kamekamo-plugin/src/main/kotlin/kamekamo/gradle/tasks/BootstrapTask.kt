...
import kamekamo.gradle.isCi
import kamekamo.gradle.isRootProject
import kamekamo.gradle.jdkVersion
import kamekamo.gradle.tasks.BootstrapPropertiesMode.APPEND
import kamekamo.gradle.tasks.BootstrapPropertiesMode.LOG
import kamekamo.gradle.tasks.BootstrapUtils.computeDaemonArgs
import kamekamo.gradle.util.mapToInt
...
constructor(objects: ObjectFactory, providers: ProviderFactory) : DefaultTask() {
    ...
    private val argsProvider = BootstrapUtils.DaemonArgsProvider.fromProviders(providers)

    // Current bootstrap version
    @get:Input
    public abstract val bootstrapVersion: Property<Int>
     ...
     */
    @get:Input
    public abstract val ciBooleanProperty: Property<Boolean>

    @get:Input
    public abstract val offlineBooleanProperty: Property<Boolean>
     ...
     */
    @get:Input
    public abstract val propertiesMode: Property<BootstrapPropertiesMode>
    ...
    @get:OutputFile
    public abstract val bootstrapVersionFileProperty: RegularFileProperty
    ...
    @get:Optional
    @get:Input
    public abstract val jdkVersion: Property<Int>
    ...
    @get:OutputFile
    public abstract val javaInstallationPath: RegularFileProperty
    ...
    internal fun bootstrap() {
        ...
        val properties =
            mutableMapOf(
                "org.gradle.jvmargs" to
                    "-Duser.country=US -Dfile.encoding=UTF-8 -XX:+HeapDumpOnOutOfMemoryError ${
                        gradleArgs.args.joinToString(
                            " "
                        )
                    }",
                KameKamoProperties.KOTLIN_DAEMON_ARGS_KEY to
                    "-Duser.country=US -Dfile.encoding=UTF-8 -XX:+HeapDumpOnOutOfMemoryError ${
                        kotlinArgs.args.joinToString(
                            " "
                        )
                    }",
            )
        ...
        val regionMarkerPrefix = "# Begin: KameKamo bootstrap-generated properties"
        val regionMarkerSuffix = "# End: KameKamo bootstrap-generated properties"
        val prefix =
            """
      $regionMarkerPrefix
        ...
        if (mode == LOG) {
            ...
            if (mode == APPEND) {
                ...
                if (existingIndex != -1) {
                    ...
                    check(endIndex != -1) {
                        "Could not find region suffix for properties, please delete the KameKamo properties below '$regionMarkerPrefix' and rerun."
                    }
        ...
        companion object {
            fun fromProviders(
                ...
                return DaemonArgsProvider(
                    ...
                    minGradleXmx =
                    providers.gradleProperty(KameKamoProperties.MIN_GRADLE_XMX).mapToInt().orElse(1),
