...
import kamekamo.dependencyrake.RakeDependencies
import kamekamo.gradle.AptOptionsConfig.AptOptionsConfigurer
import kamekamo.gradle.AptOptionsConfigs.invoke
import kamekamo.gradle.Configurations.CORE_LIBRARY_DESUGARING
import kamekamo.gradle.Configurations.KAPT
import kamekamo.gradle.Configurations.KSP
import kamekamo.gradle.dependencies.KameKamoDependencies
import kamekamo.gradle.dependencies.KotlinBuildConfig
import kamekamo.gradle.permissionchecks.PermissionChecks
...
private const val LOG = "KameKamoPlugin:"
...
internal class StandardProjectConfigurations {
    ...
    fun applyTo(project: Project) {
        ...
        val globalConfig = project.kamekamoTools().globalConfig
        project.applyJvmConfigurations(globalConfig, kameKamoProperties)
    }
    ...
    private fun Project.applyJvmConfigurations(
        ...
        val platformProjectPath = kameKamoProperties.platformProjectPath
        if (platformProjectPath == null) {
            if (kameKamoProperties.strictMode) {
                logger.warn(
                    "kamekamo.location.kamekamo-platform is not set. Consider creating one to ensure consistent dependency versions across projects!"
                )
            }
        } else if (!kameKamoProperties.noPlatform && path != platformProjectPath) {
            applyPlatforms(kameKamoProperties.versions.boms, platformProjectPath)
        }

        if (kameKamoProperties.enableAnalysisPlugin) {
            ...
            if (buildFile.exists()) {
                ...
                plugins.withId("com.autonomousapps.dependency-analysis") {
                    val isNoApi = kameKamoProperties.rakeNoApi
                    val rakeDependencies =
                        tasks.register<RakeDependencies>("rakeDependencies") {
                            ...
                            identifierMap.set(
                                project.provider {
                                    project.getVersionsCatalog(kameKamoProperties).identifierMap().mapValues { (_, v) ->
                                        "libs.$v"
                                    }
                                }
                            )
        ...
        val kamekamoExtension = extensions.create<KameKamoExtension>("kamekamo")
        checkAndroidXDependencies(kameKamoProperties)
        ...
        pluginManager.onFirst(JVM_PLUGINS) {
            kameKamoProperties.versions.bundles.commonAnnotations.ifPresent {
                dependencies.add("implementation", it)
            }

            kameKamoProperties.versions.bundles.commonTest.ifPresent {
                dependencies.add("testImplementation", it)
            }
        }

        // TODO always configure compileOptions here
        configureAndroidProjects(globalConfig, kamekamoExtension, jvmTargetVersion, kameKamoProperties)

        configureKotlinProjects(globalConfig, jdkVersion, jvmTargetVersion, kameKamoProperties)
        configureJavaProject(jdkVersion, jvmTargetVersion, kameKamoProperties)
        kamekamoExtension.configureFeatures(this, kameKamoProperties)
     ...
     */
    private fun Project.checkAndroidXDependencies(kameKamoProperties: KameKamoProperties) {
        if (!kameKamoProperties.skipAndroidxCheck) {
            configurations.configureEach {
                resolutionStrategy {
                    eachDependency {
    ...
    private fun Project.configureJavaProject(
        ...
        configureErrorProne(kameKamoProperties)
    }
     ...
     * project.
     */
    private fun Project.configureErrorProne(kameKamoProperties: KameKamoProperties) {
        val autoPatchEnabled = kameKamoProperties.errorProneAutoPatch
        pluginManager.withPlugin("net.ltgt.nullaway") {
            val nullawayBaseline = kameKamoProperties.nullawayBaseline

            val nullawayDep =
                kameKamoProperties.versions.catalog.findLibrary("errorProne-nullaway").orElseThrow {
            ...
            tasks.withType<JavaCompile>().configureEach {
                ...
                options.errorprone.nullaway {
                    ...
                    annotatedPackages.add("kamekamo")
                    ...
                    if (autoPatchEnabled && nullawayBaseline) {
                        ...
                        castToNonNullMethod.set("kamekamo.commons.JavaPreconditions.castToNotNull")
                    }
                }
            }
        }
        pluginManager.withPlugin("net.ltgt.errorprone") {
            dependencies.add("errorprone", KameKamoDependencies.ErrorProne.core)
            ...
            tasks.withType<JavaCompile>().configureEach {
                options.errorprone {
                    ...
                    disable("HidingField")
                    error(*kamekamoTools().globalConfig.errorProneCheckNamesAsErrors.toTypedArray())
    ...
    private fun Project.configureAndroidProjects(
        globalConfig: GlobalConfig,
        kamekamoExtension: KameKamoExtension,
        jvmTargetVersion: Int,
        kameKamoProperties: KameKamoProperties
    ) {
        val javaVersion = JavaVersion.toVersion(jvmTargetVersion)
        val lintErrorsOnly = kameKamoProperties.lintErrorsOnly

        val commonComponentsExtension =
            Action<AndroidComponentsExtension<*, *, *>> {
                val variantsToDisable =
                    kameKamoProperties.disabledVariants?.splitToSequence(",")?.associate {
                        ...
                        beforeVariants(selector) { builder ->
                            ...
                            if (builder is HasAndroidTestBuilder) {
                                builder.enableAndroidTest =
                                    kamekamoExtension.androidHandler.featuresHandler.androidTest.getOrElse(false)
                            }
                        }
                    }
                    if (isApp) {
        ...
        val shouldApplyCacheFixPlugin = kameKamoProperties.enableAndroidCacheFix
        val sdkVersions by lazy { kameKamoProperties.requireAndroidSdkProperties() }
        val commonBaseExtensionConfig: BaseExtension.() -> Unit = {
            ...
            kameKamoProperties.ndkVersion?.let { ndkVersion = it }
            defaultConfig {
            ...
            dependencies.add(
                ...
                KameKamoDependencies.Google.coreLibraryDesugaring
            )
        ...
        val objenesis2Version = kameKamoProperties.versions.objenesis
        ...
        val composeCompilerVersion by lazy {
            kameKamoProperties.versions.composeCompiler
                ?: error("Missing `compose-compiler` version in catalog")
        }

        pluginManager.withPlugin("com.android.base") {
            if (kameKamoProperties.enableCompose) {
                dependencies.add("implementation", KameKamoDependencies.Androidx.Compose.runtime)
            }
        }

        pluginManager.withPlugin("com.android.application") {
            // Add kamekamo-lint as lint checks source
            kameKamoProperties.versions.bundles.commonLint.ifPresent { dependencies.add("lintChecks", it) }
            ...
            configure<ApplicationAndroidComponentsExtension> {
                ...
                // TODO maybe we want to disable release androidTest by default? (i.e. for kamekamo kit
            ...
            configure<BaseAppModuleExtension> {
                ...
                lint {
                    ...
                    project
                        ...
                        .asFile
                        .takeIf { it.exists() || kameKamoProperties.lintUpdateBaselines }
                        ?.let { baseline = it }
                }
                packagingOptions {
                    resources.excludes +=
                        setOf(
                            ...
                            // https://kamekamo-pde.kamekamo.com/archives/C8EER3C04/p1621353426001500
                            "annotated-jdk/**"
                        )
                    jniLibs.pickFirsts +=
                        setOf(
                ...
                PermissionChecks.configure(
                    project = project,
                    allowListActionGetter = { kamekamoExtension.androidHandler.appHandler.allowlistAction }
                ...
                pluginManager.withPlugin("com.bugsnag.android.gradle") {
                    // See KamekamoGradleUtil.shouldEnableBugsnagPlugin for more details on this logic
            ...
            kamekamoExtension.androidHandler.configureFeatures(project, kameKamoProperties)
        }
        ...
        pluginManager.withPlugin("com.android.library") {
            ...
            kameKamoProperties.versions.bundles.commonLint.ifPresent { dependencies.add("lintChecks", it) }
            prepareAndroidTestConfigurations()
            val isLibraryWithVariants = kameKamoProperties.libraryWithVariants
            ...
            configure<LibraryAndroidComponentsExtension> {
                ...
                val androidTestApksAggregator =
                    project.rootProject.tasks.named<AndroidTestApksTask>(/* name = */ AndroidTestApksTask.NAME)
                onVariants { variant ->
                    val excluded =
                        kamekamoExtension.androidHandler.featuresHandler.androidTestExcludeFromFladle.getOrElse(
                            false
                        )
                    if (!excluded) {
    ...
    private fun Project.configureKotlinProjects(
        ...
        val onKotlinPluginApplied = {
            ...
            tasks.configureEach<KotlinCompile> {
                kotlinOptions {
                    if (!kameKamoProperties.allowWarnings && !name.contains("test", ignoreCase = true)) {
                        allWarningsAsErrors = true
                    }
                    jvmTarget = actualJvmTarget
                    freeCompilerArgs += kotlinCompilerArgs

                    if (kameKamoProperties.enableCompose && isAndroid) {
            ...
            if (kameKamoProperties.strictMode && kameKamoProperties.strictValidateKtFilePresence) {
        ...
        pluginManager.withPlugin("io.gitlab.arturbosch.detekt") {
            ...
            configure<DetektExtension> {
                toolVersion =
                    kameKamoProperties.versions.detekt ?: error("missing 'detekt' version in version catalog")
                rootProject.file("config/detekt/detekt.yml")
                kameKamoProperties.detektConfigs?.let { configs ->
                    for (configFile in configs) {
                        config.from(rootProject.file(configFile))
                    }
                }

                kameKamoProperties.detektBaseline?.let { baselineFile ->
                    baseline =
        ...
        pluginManager.withPlugin("org.jetbrains.kotlin.kapt") {
            ...
            if (!kameKamoProperties.enableKaptInTests) {
                tasks.configureEach {
