...
import kamekamo.gradle.convertProjectPathToAccessor
import kamekamo.gradle.namedLazy
...
public object ModuleStatsTasks {
    ...
    internal fun configureRoot(rootProject: Project) {
        ...
        rootProject.tasks.register<ModuleStatsAggregatorTask>(AGGREGATOR_NAME) {
            outputFile.set(rootProject.layout.buildDirectory.file("reports/kamekamo/moduleStats.json"))
    ...
    internal fun configureSubproject(project: Project) {
        if (!project.buildFile.exists() ||
            ...
            "kamekamo-platform" in project.path
        ) {
            return
        }
        ...
        val locTask =
            project.tasks.register<LocTask>("loc") {
                ...
                outputFile.set(project.layout.buildDirectory.file("reports/kamekamo/loc.json"))
            }
        ...
        val collector: Lazy<TaskProvider<ModuleStatsCollectorTask>> = lazy {
            val task =
                project.tasks.register<ModuleStatsCollectorTask>("moduleStats") {
                    ...
                    this.includeGenerated.set(includeGenerated)
                    outputFile.set(project.layout.buildDirectory.file("reports/kamekamo/moduleStats.json"))
                }
        ...
        project.pluginManager.apply {
            ...
            withPlugin("com.android.library") {
                val multiVariant = kameKamoProperties.libraryWithVariants
            ...
            project.afterEvaluate {
                val extension = the<KameKamoExtension>()
...
public abstract class ModuleStatsAggregatorTask : DefaultTask() {
    ...
    @get:Input
    internal abstract val includeGenerated: Property<Boolean>
     ...
     *
     * ```
     * services.kameKamoKitIntegrations -> :services:kamekamo-kit-integrations
     * ```
     */
    @get:Input
    internal abstract val projectPathsToAccessors: MapProperty<String, String>
...
internal abstract class ModuleStatsCollectorTask : DefaultTask() {
    ...
    @get:Input
    abstract val includeGenerated: Property<Boolean>
